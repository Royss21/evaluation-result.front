...
import {
  IComponentsRangeDate,
  IStageRangeDate,
} from '@modules/evaluation/interfaces/evaluation-detail.interface';
...
export class EvaluationDetailComponent {
  ...
  isLeaderAreaObjetive = false;
  isLeaderCompetence = false;
  logingCollaborator = '';
  ...
  ngOnInit() {
    this.logingCollaborator =
      localStorage.getItem('logingCollaborator')?.toString() || '';

    if (this.logingCollaborator === '1') {
      this.isLeaderAreaObjetive =
        localStorage.getItem('isLeaderAreaObjetive') == 'true' ? true : false;
      this.isLeaderCompetence =
        localStorage.getItem('isLeaderCompetence') == 'true' ? true : false;
  ...
  get infoObjetivesCorporate(): IComponentsRangeDate | null {
    return (
      this.components.find(
        (c) => c.componentId === ConstantsGeneral.components.corporateObjectives
      ) || null
    );
  }
  ...
  get infoObjetivesArea(): IComponentsRangeDate | null {
    return (
      this.components.find(
        (c) => c.componentId === ConstantsGeneral.components.areaObjectives
      ) || null
    );
  }
  ...
  get infoCompetence(): IComponentsRangeDate | null {
    return (
      this.components.find(
        (c) => c.componentId === ConstantsGeneral.components.competencies
      ) || null
    );
  }
      ...
      .subscribe((stageId) => {
        if (
          ...
          this._router.navigateByUrl(
            `/evaluation/${this._evaluationId}/evaluate-component/${ConstantsGeneral.components.competencies}`
          );
        else if (
          [
          ...
          this._router.navigateByUrl(
            `/evaluation/${this._evaluationId}/review-stage/${stageId}`
          );
      });
  }
  ...
  goToEvaluate(componentId: any) {
    if (componentId != '0')
      this._router.navigateByUrl(
        `/evaluation/${this._evaluationId}/evaluate-component/${componentId}`
      );
  }

  goToCollaborators() {
    this._router.navigateByUrl(
      `/evaluation/${this._evaluationId}/collaborator`
    );
  }
