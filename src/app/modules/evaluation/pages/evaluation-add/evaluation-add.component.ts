...
export class EvaluationAddComponent {
  ...
  stagesEvaluation: number[] = [
    ConstantsGeneral.stages.feedback,
    ConstantsGeneral.stages.approval,
  ];
  stagesCompetence: number[] = [
    ConstantsGeneral.stages.evaluation,
    ConstantsGeneral.stages.calibration,
  ];
  ...
  constructor(
    ...
    this.evaluationFormGroup =
      this._evaluationBuilderService.buildEvaluationForm();
    ...
    this._route.params.subscribe((params) => {
      ...
      if (this.evaluationId) {
        ...
        this._evaluationService
          .getDatesComponentes(this.evaluationId ?? '')
          .subscribe((data) => {
            this.controlsEvaluationForm['name'].setValue(data.name);
            this.controlsEvaluationForm['weight'].setValue(data.weight);
            this.controlsEvaluationForm['startDate'].setValue(
              this._toDate(data.startDate)
            );
            this.controlsEvaluationForm['endDate'].setValue(
              this._toDate(data.endDate)
            );
            ...
            data.componentStagesDates.forEach((c) => {
              if (!c.componentId) {
                if (c.stageId === ConstantsGeneral.stages.feedback) {
                  this.stageFeedback
                    .get('startDate')
                    ?.setValue(this._toDate(c.startDate));
                  this.stageFeedback
                    .get('endDate')
                    ?.setValue(this._toDate(c.endDate));
              ...
              if (
                ...
                c.componentId === ConstantsGeneral.components.areaObjectives
              ) {
                ...
                c.componentId === ConstantsGeneral.components.competencies
              ) {
  ...
  private _getCurrentDatesPeriod(): void {
    this._periodService.getCurrentDatePeriod().subscribe((period) => {
  ...
  get evaluationComponentStages(): FormArray {
    return this.evaluationFormGroup.get(
  ...
  private _save(evaluation: IEvaluationCreate) {
    if (this.evaluationId)
      this._evaluationService
        .update(evaluation, this.evaluationId)
        .subscribe((evaluation) => {
          this._showConfirmMessage(evaluation.id);
        });
    else
      this._evaluationService.create(evaluation).subscribe((evaluation) => {
        this._showConfirmMessage(evaluation.id);
      });
  }
  ...
  private _onChangesValuesForm(): void {
    this.controlsEvaluationForm['startDate'].valueChanges.subscribe((value) => {
      ...
      } else this.minDateEvaluation = this.startDatePeriod;
    });
    ...
    this.controlsEvaluationForm['endDate'].valueChanges.subscribe((value) => {
      if (value) {
        ...
        this._compareDates('evaluation');
      } else this.maxDateComponentsEvaluation = this.endDatePeriod;
    });
    ...
    this.corpGoal.get('startDate')?.valueChanges.subscribe((value) => {
      if (value) this._compareDates('corpGoals');
    });
    ...
    this.areaGoal.get('startDate')?.valueChanges.subscribe((value) => {
      if (value) this._compareDates('areaGoals');
    });
    ...
    this.stageApproval.get('startDate')?.valueChanges.subscribe((value) => {
      if (value) this._compareDates('stageApproval');
    });
    ...
    this.stageFeedback.get('startDate')?.valueChanges.subscribe((value) => {
      if (value) this._compareDates('stageFeedback');
    });
    ...
    this.stageEvaluation.get('startDate')?.valueChanges.subscribe((value) => {
      if (value) this._compareDates('stageEvaluation');
    });
    ...
    this.stageCalibration.get('startDate')?.valueChanges.subscribe((value) => {
      if (value) this._compareDates('stageCalibration');
    });
  ...
  private _compareDates(nameForm: string) {
    switch (nameForm) {
      case 'evaluation':
        if (
          this._toDate(
            this.controlsEvaluationForm['startDate']?.value
          )?.getTime() >=
          this._toDate(this.controlsEvaluationForm['endDate']?.value)?.getTime()
        )
          this.controlsEvaluationForm['endDate'].setErrors({
            invalidDate: true,
          });
        else this.controlsEvaluationForm['endDate'].setErrors(null);
      ...
      case 'corpGoals':
        ...
        else this.corpGoal.get('endDate')?.setErrors(null);
      ...
      case 'areaGoals':
        ...
        if (
          this._toDate(startDateArea)?.getTime() >=
          this._toDate(endDateArea)?.getTime()
        )
          this.areaGoal.get('endDate')?.setErrors({ invalidDate: true });
        else this.areaGoal.get('endDate')?.setErrors(null);
      ...
      case 'stageApproval':
        ...
        if (
          this._toDate(startDateAproval)?.getTime() >=
          this._toDate(endDateAproval)?.getTime()
        )
          this.stageApproval.get('endDate')?.setErrors({ invalidDate: true });
        else this.stageApproval.get('endDate')?.setErrors(null);
      ...
      case 'stageFeedback':
        ...
        if (
          this._toDate(startDateFeedback)?.getTime() >=
          this._toDate(endDateFeedback)?.getTime()
        )
          this.stageFeedback.get('endDate')?.setErrors({ invalidDate: true });
        else this.stageFeedback.get('endDate')?.setErrors(null);

        break;

      case 'stageEvaluation':
        const startDateEvaluation =
          this.stageEvaluation.get('startDate')?.value;
        const endDateEvaluation = this.stageEvaluation.get('endDate')?.value;
        if (
          this._toDate(startDateEvaluation)?.getTime() >=
          this._toDate(endDateEvaluation)?.getTime()
        )
          this.stageEvaluation.get('endDate')?.setErrors({ invalidDate: true });
        else this.stageEvaluation.get('endDate')?.setErrors(null);

        break;

      case 'stageCalibration':
        const startDateCalibration =
          this.stageCalibration.get('startDate')?.value;
        const endDateCalibration = this.stageCalibration.get('endDate')?.value;
        if (
          this._toDate(startDateCalibration)?.getTime() >=
          this._toDate(endDateCalibration)?.getTime()
        )
          this.stageCalibration
            .get('endDate')
            ?.setErrors({ invalidDate: true });
        else this.stageCalibration.get('endDate')?.setErrors(null);
  ...
  confirmSave(): void {
    const evaluationCreate: IEvaluationCreate = {
      ...this.evaluationFormGroup.getRawValue(),
    };
    evaluationCreate.evaluationComponents = [
      ...evaluationCreate.evaluationComponents.filter((ec) => ec.checked),
    ];
    ...
    if (evaluationCreate.evaluationComponents.length <= 0) {
      dataPopup.text =
        'Debe seleccionar un componente para crear la evaluación';
      const dialogRefConfirm = this._dialog.open(PopupConfirmComponent, {
        data: dataPopup,
        autoFocus: false,
      });
    ...
    if (this.evaluationFormGroup.invalid) return;

    if (
      !evaluationCreate.evaluationComponents.some(
        (ec) => ec.componentId === ConstantsGeneral.components.competencies
      )
    )
      evaluationCreate.evaluationComponentStages = [
        ...evaluationCreate.evaluationComponentStages.filter(
          (s) => s.componentId !== ConstantsGeneral.components.competencies
        ),
      ];
    else {
      evaluationCreate.evaluationComponents =
        evaluationCreate.evaluationComponents.map((ec) =>
          ec.componentId === ConstantsGeneral.components.competencies
            ? {
                ...ec,
                startDate:
                  evaluationCreate.evaluationComponentStages.find(
                    (s) =>
                      s.componentId ===
                        ConstantsGeneral.components.competencies &&
                      s.stageId === ConstantsGeneral.stages.evaluation
                  )?.startDate || null,
                endDate:
                  evaluationCreate.evaluationComponentStages.find(
                    (s) =>
                      s.componentId ===
                        ConstantsGeneral.components.competencies &&
                      s.stageId === ConstantsGeneral.stages.calibration
                  )?.endDate || null,
              }
            : ec
        );
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) this._save(evaluationCreate);
    });
  }
}
