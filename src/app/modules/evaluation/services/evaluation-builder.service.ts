...
export class EvaluationBuilderService {
  ...
  public buildEvaluationForm(): FormGroup {
    ...
    const evaluationComponentStages =
      EvaluationHelper.evaluationComponentStages;
    ...
    return this._fb.group({
      ...
      isEvaluationTest: [false, [Validators.required]],
      evaluationComponents: this._fb.array(
        evaluationComponents.map((c) => this.createComponent(c))
      ),
      evaluationComponentStages: this._fb.array(
        evaluationComponentStages.map((c) => this.createStage(c))
      ),
    });
  }
