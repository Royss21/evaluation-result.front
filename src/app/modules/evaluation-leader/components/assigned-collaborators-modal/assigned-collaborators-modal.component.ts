...
import {
  LeaderHelper,
  LeaderText,
} from '@modules/evaluation-leader/helpers/leader.helper';
...
export class AssignedCollaboratorsModalComponent
  implements OnInit, AfterViewInit
{
  //AUN FALTA CULMINAR EL LISTADO Y HACER PRUEBAS

  @ViewChildren('elementObserver', { read: ElementRef })
  elementObserver: QueryList<ElementRef>;

  private filterCurrent: ILeaderCollaboratorFilter;
  private observer: any;
  private countCurrent = 0;
  countTotal = 0;
  ...
  ngAfterViewInit(): void {
    this.elementObserver.changes.subscribe((e) => {
      console.log('aaaa');
      if (e.last) this.observer.observe(e.last.nativeElement);
    });
  }
