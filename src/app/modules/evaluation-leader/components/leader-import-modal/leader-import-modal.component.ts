...
export class LeaderImportModalComponent implements OnInit {
  ...
  hasComponentAreaObjectives = false;
  hasComponentCompentences = false;
  ...
  constructor(
    ...
    this.leaderImportFormGroup =
      _leaderImportBuilderService.buildLeaderImportForm();
  }
  ...
  private _getEnabledComponents() {
    this._evaluationService
      ...
      .subscribe((value) => {
        ...
        this.leaderImportFormGroup
          .get('typeImportLeaders')
          ?.setValue(
            this.hasComponentCompentences
              ? TypeImportLeader.competencies
              : TypeImportLeader.areaObjectives
          );
      });
  }
  ...
  confirmSave() {
    ...
    if (this.leaderImportFormGroup.invalid) return;
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) this._save(leaderImport);
    });
  }
